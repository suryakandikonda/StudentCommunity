...
<html lang="en">
  <head>
    ...
    <?php
    $sql = "SELECT * FROM literaryclubuploads WHERE cid='".$_GET['id']."'";
    $result = mysqli_query($conn,$sql);
      while($row = $result->fetch_assoc()){
    echo '
    <title>'.$row["title"].' - Literary Club - URS Community</title>
  </head>
  <body>
    <!--NAV BAR-->
  ...
  echo '<h5> &#8629;    <a href="literaryclub.php">Go back to Club Page</a></h5>';
      }
?>
        ...
        <?php 
        /*
  $sql = "SELECT * FROM literaryclubuploads ORDER BY cid DESC LIMIT 2";
    $result = mysqli_query($conn,$sql);
      while($row = $result->fetch_assoc()) {
        $content = $row['content'];
